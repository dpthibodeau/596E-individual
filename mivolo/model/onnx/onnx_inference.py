...
class ONNXInference:
        ...
        for i in range(detection.shape[0]):
            ...
            if confidence > self.confidence_threshold:
                ...
                if (
                    ...
                    and height >= self.min_face_size
                    and width <= self.max_face_size
                    and height <= self.max_face_size
                    and 0.000005 < area_ratio < 0.3
                    and abs(width - height) / max(width, height) < 0.3
                ):
    ...
    def recognize(self, img):
        ...
        if not all_boxes:
            return (
                DetectedObjects(Results(Boxes(xyxy=torch.tensor([]))), [], [], {}),
                img,
            )
        ...
        if len(boxes) > 2:
            ...
            clustering = DBSCAN(eps=0.25, min_samples=1).fit(
                face_centers.reshape(-1, 1)
            )
        ...
        keep_indices = self.nms_with_overlap_threshold(
            boxes, scores, num_faces=num_clusters
        )
        ...
        age_gaps = [
            sorted_ages[i + 1] - sorted_ages[i] for i in range(len(sorted_ages) - 1)
        ]
        ...
        if len(age_gaps) > 0:
            ...
            for i in range(len(normalized_ages)):
                if (
                    i > 0
                    and normalized_ages[i] - normalized_ages[i - 1] < median_gap / 2
                ):
