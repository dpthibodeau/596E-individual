...
class PersonAndFaceCrops:
    ...
    def get_faces_with_bodies(
        self, use_person_crops: bool, use_face_crops: bool
    ) -> Tuple[Tuple[List[int], List[np.ndarray]], Tuple[List[int], List[np.ndarray]]]:
...
class PersonAndFaceResult:
    def __init__(self, yolo_results):
        ...
        self.boxes = (
            yolo_results.boxes.xyxy.cpu().numpy() if len(yolo_results.boxes) > 0 else []
        )
        self.classes = (
            yolo_results.boxes.cls.cpu().numpy() if len(yolo_results.boxes) > 0 else []
        )
        self.conf = (
            yolo_results.boxes.conf.cpu().numpy() if len(yolo_results.boxes) > 0 else []
        )
    ...
    def associate_faces_with_persons(self):
        ...
        for face_idx in face_indices:
            ...
            for person_idx in person_indices:
                ...
                if (
                    face_center[0] >= person_box[0]
                    and face_center[0] <= person_box[2]
                    and face_center[1] >= person_box[1]
                    and face_center[1] <= person_box[3]
                ):
                    self.face_to_person_map[face_idx] = person_idx
                    break

    def collect_crops(
        self, image: np.ndarray, size: Tuple[int, int] = (224, 224)
    ) -> "PersonAndFaceCrops":
    ...
    def plot(
        self, image: Optional[np.ndarray] = None, show_conf: bool = True
    ) -> np.ndarray:
        ...
        colors = {0: (255, 0, 0), 1: (0, 255, 0)}  # person - blue  # face - green
        ...
        for i, box in enumerate(self.boxes):
            ...
            cv2.rectangle(
                image, (x1, y1 - text_size[1] - 3), (x1 + text_size[0], y1), color, -1
            )
